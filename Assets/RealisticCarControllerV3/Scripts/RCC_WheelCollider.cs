...
public class RCC_WheelCollider : MonoBehaviour {
	...
	void  FixedUpdate (){
		...
		wheelRPMToSpeed = (((wheelCollider.rpm * wheelCollider.radius) / 2.8f) * Mathf.Lerp(1f, .75f, hit.forwardSlip)) * carRigid.transform.lossyScale.y;
	...
	public void WheelAlign (){
		...
		if(Physics.Raycast(ColliderCenterPoint, -wheelCollider.transform.up, out hit, (wheelCollider.suspensionDistance + wheelCollider.radius) * transform.localScale.y) && hit.transform.gameObject.layer != (int)Mathf.Log(RCC_Settings.Instance.vehicleLayer.value, 2) && !hit.collider.isTrigger){
//			if(hit.transform.gameObject.layer == (int)Mathf.Log(RCC_Settings.Instance.vehicleLayer.value, 2))
//				return;
			...
			Debug.DrawLine(CorrespondingGroundHit.point, CorrespondingGroundHit.point + wheelCollider.transform.up * (CorrespondingGroundHit.force / carRigid.mass), extension <= 0.0 ? Color.magenta : Color.white);
	...
	public void Drift(float forwardSlip){
		...
		if(wheelCollider == carController.FrontLeftWheelCollider.wheelCollider || wheelCollider == carController.FrontRightWheelCollider.wheelCollider){
			...
			forwardFrictionCurve.asymptoteValue = Mathf.Clamp(.75f - (sqrVel / 2f), .8f,  minForwardStiffness);
		}
