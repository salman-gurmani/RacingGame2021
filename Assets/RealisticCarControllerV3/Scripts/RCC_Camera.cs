...
public class RCC_Camera : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (playerCar.currentGear == 0&&playerCar.throttleInput>0)
        {
            thisCam.fieldOfView = Mathf.MoveTowards(thisCam.fieldOfView, targetFieldOfView, Time.deltaTime * 10f);
            TPSDistance = Mathf.MoveTowards(TPSDistance, 8, 0.01f);
        }
        else
        {
            thisCam.fieldOfView = Mathf.Lerp(thisCam.fieldOfView, targetFieldOfView, Time.deltaTime * 3f);
            TPSDistance = Mathf.MoveTowards(TPSDistance, 6, 0.01f);
        }
    }
    ...
    private void TPS()
    {
        ...
        if ((playerCar.currentGear == 0) && playerCar.throttleInput > 0)
            targetFieldOfView = Mathf.MoveTowards(TPSMinimumFOV, TPSMaximumFOV - 5, 10f);
        else
