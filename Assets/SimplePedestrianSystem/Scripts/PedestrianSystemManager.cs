...
namespace PedestrianSystem{
	...
	public class PedestrianSystemManager : MonoBehaviour {
		...
		public int newWaypointOffset_X = 7;
		public int newWaypointOffset_Y = 0;
		public int newWaypointOffset_Z = 0;
		...
		[Header ("Pooling")]
		public List <GameObject> pedestriansPool;
		//public Transform poolPediHoldPoint;
		...
		void Start(){
			...
			if (usePooling) {
				...
				//InstantiatePoolPedestrians();

				StartCoroutine(InstantiatePoolPedestrians());
			}
		}
		...
		IEnumerator InstantiatePoolPedestrians()
		{

			for (int i = 0; i < maxSpawnLimit; i++)
			{
				pedestriansPool.Add((GameObject)Instantiate(pedestrians[Random.Range(0, pedestrians.Length)], Vector3.zero, Quaternion.identity));
				pedestriansPool[i].SetActive(false);

				pedestriansPool[i].GetComponent<Pedestrian>().Spawner = spawner;
		...
		//void InstantiatePoolPedestrians() {
  ...
  //          {
		...
		public GameObject Get_AvailablePoolPedestrian()
		{
            for (int i = 0; i < pedestriansPool.Count; i++)
            {
				if (!pedestriansPool[i].activeSelf) {

					//Debug.Log("Returning = " + i);
					return pedestriansPool[i];
				}
            }

			//It will never come here.
			return pedestriansPool[0];
		}
		...
		public void AddWaypoint(){
			...
			GameObject obj = (GameObject) Instantiate (waypoint, temp.position + new Vector3(newWaypointOffset_X, newWaypointOffset_Y, newWaypointOffset_Z) , temp.rotation);
			obj.transform.parent = this.transform;

			//GameObject[] objArray = { obj };
