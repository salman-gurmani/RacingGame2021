...
namespace InsaneSystems.RoadNavigator
{
	public class Map : MonoBehaviour, IDragHandler
	{
		...
		void Update()
		{
			...
			playerIconOnMap.localEulerAngles = new Vector3(0, 0, -navigatorPlayerTransform.localEulerAngles.y + storage.busIconAngleOffset);
			...
			if (workAsNavigator)
			{
				...
				mapTransform.localEulerAngles = new Vector3(0, 0, (storage.busIconAngleOffset * 2) - playerIconOnMap.localEulerAngles.z - storage.busIconAngleOffset);

				playerIconOnMap.anchoredPosition = mapPosition;
				playerIconOnMap.localEulerAngles = new Vector3(0, 0, storage.busIconAngleOffset);
